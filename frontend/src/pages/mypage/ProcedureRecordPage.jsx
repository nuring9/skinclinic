...
export default function ProcedureRecordPage() {
  ...
  const loadData = async () => {
    const [candidateData, reviewData] = await Promise.all([
      getProcedureReviewCandidates(USER_ID),
      getUserProcedureReviews(USER_ID),
    ]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      alert("시술 만족도 평가가 DB에 저장되었습니다.");
      setForm(initialForm);
      await loadData();
    } catch (error) {
      console.error("시술 만족도 평가 등록 실패", error);
      alert("시술 만족도 평가 등록에 실패했습니다.");
    }
  };
  ...
  return (
    <section className="mypage-section-card">
      <div className="notification-header">
        <div>
          <h2>시술 기록 및 만족도 평가</h2>
          <p className="notification-subtitle">
            DB에 저장된 완료 시술 기록을 불러와 별점과 후기를 남깁니다.
          </p>
        </div>
      </div>
            ...
            <div key={item.id} className="mypage-section-item procedure-review-history-card">
              ...
              <div className="procedure-review-score-grid">
                <div className="procedure-review-score-item">
                  <span>효과 만족도</span>
                  {item.effectSatisfaction
                    ? renderReviewStars(item.effectSatisfaction)
                    : <strong>-</strong>}
                </div>
                <div className="procedure-review-score-item">
                  <span>가격 만족도</span>
                  {item.priceSatisfaction
                    ? renderReviewStars(item.priceSatisfaction)
                    : <strong>-</strong>}
                </div>
                <div className="procedure-review-score-item">
                  <span>상담 만족도</span>
                  {item.consultationSatisfaction
                    ? renderReviewStars(item.consultationSatisfaction)
                    : <strong>-</strong>}
                </div>
                <div className="procedure-review-score-item">
                  <span>재방문 의사</span>
                  {item.revisitIntention
                    ? renderReviewStars(item.revisitIntention)
                    : <strong>-</strong>}
                </div>
              </div>
